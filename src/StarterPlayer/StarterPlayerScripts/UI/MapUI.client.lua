...
local MINIMAP_CONFIG = {
	...
	-- 任务图标配置
	QUEST_ICON = {
		IMAGE_ID = "rbxassetid://134282935527860",
	},
	-- 地图标记配置
	MAP_FLAG_ICON = {
...
local playerIcons = {}
local npcIcons = {}
local _isLoadQuest = false

local _screenGui = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("MapUI")
...
local function worldToImageLabelPosition(worldPos, imageLabel)
	local worldSize = MINIMAP_CONFIG.WORLD_SIZE[_G.ClientData.IslandId]
	local orient = MINIMAP_CONFIG.ORIENTATION[_G.ClientData.IslandId] or { FLIP_X = false, FLIP_Z = true, ROTATE_DEG = 0 }
...
local function centerBigMapOnWorldPosition(worldPosition)
	...
	local orient = MINIMAP_CONFIG.ORIENTATION[_G.ClientData.IslandId] or { FLIP_X = true, FLIP_Z = true, ROTATE_DEG = 0 }
...
local function playCircleEffect(icon)
	...
	for i = 1, 3 do
		...
		circle.Size = UDim2.new(0, icon.Size.X.Offset * 10, 0, icon.Size.Y.Offset * 10)
		-- 动画目标：缩小到icon大小，逐渐透明
		local tween = TweenInterface.TweenNode(circle, TweenInfo.new(0.5, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), {
			Size = icon.Size,
			ImageTransparency = 1
		})
...
local function createPlayerIcon(player, parent)
	...
	local isSmall = (parent == _smallImageLabel) or (parent == _smallMap)
...
-- @param mapFrame Frame 小地图框架
-- @return ImageLabel 创建的图标
local function createNPCIcon(npc, parent)
	local icon = Instance.new("ImageLabel")
	icon.Name = "NPCIcon_" .. npc.Name
	icon.Image = MINIMAP_CONFIG.NPC_ICON.IMAGE_ID
	icon.Size = (parent == _smallImageLabel) and MINIMAP_CONFIG.SMALL_SIZE or MINIMAP_CONFIG.BIG_SIZE
	icon.AnchorPoint = MINIMAP_CONFIG.ANCHOR_POINT
	icon.BackgroundTransparency = 1
	icon.Parent = parent
...
-- @param mapFrame Frame 小地图框架
-- @return ImageLabel 创建的图标
local function createQuestIcon(pos, parent)
	local icon = Instance.new("ImageLabel")
	icon.Name = "QuestIcon"
	icon.Image = MINIMAP_CONFIG.QUEST_ICON.IMAGE_ID
	icon.Size = (parent == _smallImageLabel) and MINIMAP_CONFIG.SMALL_SIZE or MINIMAP_CONFIG.BIG_SIZE
	icon.AnchorPoint = MINIMAP_CONFIG.ANCHOR_POINT
	icon.BackgroundTransparency = 1
	icon.Parent = parent
	icon.Position = worldToImageLabelPosition(pos, parent)
...
Knit.OnStart():andThen(function()
	--local island = Interface.safeWaitPart(workspace, _G.ClientData.IslandId)
	...
	RunService.Heartbeat:Connect(function(dt)
		...
		--		updatePlayerIcon(player, playerIcons[player].small, _smallImageLabel)
		...
		--local questData = _G.ClientData.QuestData
		--if questData then
		--	if not _isLoadQuest then
		--		_isLoadQuest = true
		...
		--				if tp.Type == GameConfig.TaskType.RetrieveAtLocation 
		--					or tp.Type == GameConfig.TaskType.PlaceAtLocation 
		--					or tp.Type == GameConfig.TaskType.ScoutArea then
		--					createQuestIcon(tp.Pos, _smallImageLabel)
		--					createQuestIcon(tp.Pos, _bigImageLabel)
		--				end
		--			end
		--		end
		--	end
