...
local _centerFrame = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("MainUI"):WaitForChild("center")
local _itemImageLabel = _centerFrame:WaitForChild("ImageLabel")
...
local function PlayItemImageFade(imageLabel, fadeIn, hold, fadeOut)
	...
	-- 淡入
	_itemImageFadeTween = TweenInterface.TweenNodeTransparencyImage(imageLabel, 0, fadeIn, function()
		if _itemImageFadeSeqId ~= seqId then return end
		_itemImageFadeConn = nil
		-- 停留 -> 淡出（使用 task.delay）
		_itemImageFadeDelayThread = task.delay(hold, function()
			if _itemImageFadeSeqId ~= seqId then return end
			_itemImageFadeTween = TweenInterface.TweenNodeTransparencyImage(imageLabel, 1, fadeOut, function()
				if _itemImageFadeSeqId ~= seqId then return end
				_itemImageFadeConn = nil
				_itemImageFadeTween = nil
